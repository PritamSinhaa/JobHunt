      ...
      <div className='max-w-4xl mx-auto bg-white border border-gray-200 rounded-2xl my-5 p-8'>
        ...
        <div className='my-5'>
          ...
          <div className="flex items-center gap-1">
            {
              user?.profile?.skills.length !== 0 ? user?.profile?.skills.map((item, index) => (<Badge key={index} variant='outline'>{item}</Badge>)) : <span>N/A</span>
            }
          </div>
        </div>
        <div className="grid w-full max-w-small items-center gap-1.5">
          <Label className='text-md font-bold'>Resume</Label>
          {
            isResume ? <a target='blank' href={user?.profile?.resume}
              className='text-blue-500 w-full hover:underline cursor-pointer'>{user?.profile?.resumeOriginalName}</a> : <span>N/A</span>
          }
